...
{
    int i = 0;
    for(i; i < FIELD_STR && g <= GOLD_CARD; ++i)
    {
        field[INFORM][i] = 0;
        if(field[JEWEL][i] == 0)
            field[JEWEL][i] = gold[g++];
        field[BURG][i] = 0;
    }
    if(i != FIELD_STR)
        return false;
...
SDL_Texture* LoadImage(const char *file, SDL_Renderer *ren)
{
   ...
   if (loadedImage != NULL){
       ...
       SDL_FreeSurface(loadedImage);
        ...
        printf("SDL_GetError(): loadedImage\n");
   return texture;
}
...
void DrawInform(const SDL_Texture *pic, const int *mas)
{
...
void DrawBurg(const SDL_Texture *pic, const int *mas)
{
    for(int i = 0; i < BUILDINGS; ++i)
    {
        int x = CARD_X + i * CARD_W;
...
void DrawGold(const SDL_Texture *pic, const int *mas)
{
    for(int i = 0; i < BUILDINGS; ++i)
    {
        int x = CARD_X + i * CARD_W;
...
void Show_field(int **field, SDL_Texture *pic, SDL_Renderer ren)
{
    ...
    DrawInform(pic, field[INFORM]);
    DrawPlace(pic);
    DrawGold(pic, field[GOLD]);
    DrawBurg(pic, field[BURG]);
